...
const Map<String, String> FESTIVAL = {
  '1-1': '春节',
  '1-15': '元宵节',
  '2-2': '龙头节',
  '5-5': '端午节',
...
/**
      ...
      * @return Number
      */
List solarMonth = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
...
int getTerm(int y, int n) {
  ...
  List<String> _info = [
    int.parse('0x' + _table.substr(0, 5)).toString(),
    int.parse('0x' + _table.substr(5, 5)).toString(),
    int.parse('0x' + _table.substr(10, 5)).toString(),
    int.parse('0x' + _table.substr(15, 5)).toString(),
    int.parse('0x' + _table.substr(20, 5)).toString(),
    int.parse('0x' + _table.substr(25, 5)).toString()
  ];
  List<String> _calday = [
    _info[0].substr(0, 1),
    _info[0].substr(1, 2),
...
Map solar2lunar(int y, int m, int d) {
  ...
  if (y == null) {
    ...
    String formatStr =
        y.toString() + twoDigets(m.toString()) + twoDigets(d.toString());
    objDate = DateTime.parse(formatStr);
  }
  var i, leap = 0, temp = 0;
  //修正ymd参数
  y = objDate.year;
  m = objDate.month;
  d = objDate.day;
  var offset = (objDate.millisecondsSinceEpoch -
          DateTime.parse("19000131").millisecondsSinceEpoch) /
      86400000;
  ...
  if (isTodayObj.year == y &&
      isTodayObj.month + 1 == m &&
      isTodayObj.day == d) {
    isToday = true;
  }
  //星期几
  var nWeek = objDate.weekday, cWeek = nStr1[nWeek];
  ...
  String? festival;
  String? _festival;
  festival = FESTIVAL[month.toString() + '-' + (day ~/ 1).toString()];
  _festival = FESTIVAL[month.toString() + '-' + (day ~/ 1).toString()];
  if (festival == null) {
    festival = Term;
  }
  if (_festival == null) {
  ...
  if (festival == null) {
    festival = toChinaDay(day ~/ 1);
    if (festival == "初一") {
      festival = (isLeap ? "\u95f0" : '') + toChinaMonth(month);
    }
  }
